...
//! browser (feature-inventory §3.5; port of keel's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.keel/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `keel/<name>` branch. `KEEL_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
    "otter", "harbor", "falcon", "cedar", "meadow", "keel", "delta", "ember", "lynx", "maple",
...
/// worktrees are user-facing working checkouts. `KEEL_WORKTREES_DIR` overrides
/// (test isolation); empty reads as unset.
fn default_worktrees_root() -> PathBuf {
    std::env::var_os("KEEL_WORKTREES_DIR")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".keel").join("worktrees"))
}
...
impl Repos {
    ...
    /// comes from `$KEEL_WORKTREES_DIR` or `~/.keel/worktrees`.
    ...
    /// remotes, and tags seed the walk, so Keel's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `keel/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists() && !existing.contains(&format!("keel/{candidate}")) {
                name = Some(candidate);
                break;
            }
        }
        ...
        let branch_name = format!("keel/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a keel-created worktree branch after its chat's generated title
    /// (port of keel's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `keel/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("keel/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when keel created it (`keel/…`) — the
    ...
    /// timeout the thread is simply abandoned (the keel backend's disposable
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
    let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
        std::io::ErrorKind::PermissionDenied => {
            EngineError::Other("Keel doesn't have access to this folder on the device.".into())
        }
...
/// Turn a generated chat title into the semantic portion of a Keel branch
/// (port of keel's `worktreeBranchFromTitle`). Keel NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
    ...
    format!("keel/{}", if slug.is_empty() { "update" } else { slug })
}
