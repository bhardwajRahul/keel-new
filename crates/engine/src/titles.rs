...
//! untitled chat, name it with the harness's cheapest model (port of keel's
...
//!    last listed model — keel's `cheapestModel`);
...
//!    retrying on keel's short backoff ladder; fall back to the prompt's first
//!    words when every attempt produces nothing;
//! 4. re-check the title (a user rename during generation wins);
//! 5. when the chat sits in a keel worktree (`keel/<name>` branch), rename the
...
use keel_harness::{CancellationToken, RunControls, SteerMessage};
use keel_proto::{
...
/// couple of times with a short backoff before falling back (keel's ladder).
...
impl TitleGenerator {
    ...
    async fn generate(
        ...
        // keel/<name> branch (guards live inside rename_worktree_branch).
        if let (Some(chat_cwd), Some(branch)) = (&latest.cwd, &latest.branch)
            && branch.starts_with("keel/")
        {
            match self
                .inner
                .repos
...
/// The cheapest model a harness offers (keel's `cheapestModel` heuristic):
...
async fn collect_text(
    harness: &dyn keel_harness::Harness,
    request: RunRequest,
