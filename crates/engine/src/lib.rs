//! keel-engine — the headless backend: sessions engine, doc host + command executor,
...
pub use keel_proto::{EngineInfo, HarnessId, WorkspaceScope};
use keel_rpc::{RpcError, RpcReply, RpcService, methods};

use keel_sync::DocsStore;
...
pub use registry::{
    HarnessDescriptor, HarnessRegistry, default_registry, default_registry_for_data_dir,
};
...
pub enum EngineError {
    ...
    #[error("store: {0}")]
    Store(#[from] keel_sync::StoreError),
    #[error("harness: {0}")]
    Harness(#[from] keel_harness::HarnessError),
    #[error("io: {0}")]
...
pub struct EngineConfig {
    /// Data directory (default `~/.keel`, dev `~/.keel-dev`).
    ...
    /// Workspace-doc org (`ws/{orgId}` room). `None` = `$KEEL_ORG_ID` or the dev default.
...
pub struct EngineCore {
    ...
    links: std::sync::Mutex<Option<Arc<keel_rpc::LinkCache>>>,
    /// Release checker (attached by [`Engine::assemble_runtime`]) — the
    /// UpdateStatus stream + ApplyUpdate.
    updater: std::sync::Mutex<Option<keel_update::Updater>>,
...
impl EngineCore {
    ...
    /// `$KEEL_ORG_ID` / `$KEEL_USER_ID` (dev defaults `dev-org` / `dev-user`);
    ...
    pub fn assemble(
        ...
        let org_id = env_or("KEEL_ORG_ID", DEFAULT_ORG_ID);
        let user_id = env_or("KEEL_USER_ID", DEFAULT_USER_ID);
    ...
    pub fn assemble_with_profile_locked(
        ...
        sessions.set_decision_data_dir(data_dir.to_path_buf());
    ...
    pub fn auth(&self) -> Auth {
        ...
        slot.get_or_insert_with(|| {
            let dev_user = std::env::var("KEEL_EDGE_TOKEN")
                .ok()
    ...
    pub fn set_links(&self, links: Arc<keel_rpc::LinkCache>) {
        *self
            .links
            .lock()
    ...
    pub fn links(&self) -> Option<Arc<keel_rpc::LinkCache>> {
        self.links
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    pub fn set_updater(&self, updater: keel_update::Updater) {
        *self
            .updater
            .lock()
    ...
    pub fn updater(&self) -> Option<keel_update::Updater> {
        self.updater
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    pub async fn dial_device(
        &self,
        device_id: &str,
    ) -> Result<Arc<keel_rpc::RpcClient>, EngineError> {
        let links = self
            .links()
    ...
    pub fn start_host_relay(&self, edge_url: &str) -> keel_rpc::HostRelay {
        let auth = self.auth();
        let config =
            keel_rpc::HostRelayConfig::new(edge_url, self.device_id.clone(), Arc::new(auth));
        let doc_host = self.doc_host.clone();
        let on_nudge: keel_rpc::NudgeHandler = Arc::new(move |chat_id: String| {
        ...
        keel_rpc::HostRelay::spawn(config, self.rpc_service(), on_nudge)
    }
...
pub struct EngineRuntime {
    core: EngineCore,
    host_relay: std::sync::Mutex<Option<keel_rpc::HostRelay>>,
}

/// IPC-only lifecycle control owned by `keel headless`. The regular
...
impl Engine {
    ...
    pub async fn build_auth(config: &EngineConfig) -> Auth {
        ...
        if let Ok(base) = std::env::var("KEEL_WORKOS_API_BASE")
            && !base.trim().is_empty()
        {
            auth_config.workos_api_base = base;
        }
        auth_config.callback_port = Some(
            std::env::var("KEEL_CALLBACK_PORT")
                .ok()
    ...
    /// A fresh Keel install without cloud credentials owns a true local
    /// profile. Explicit development bearer setups retain the inherited
    /// development profile for compatibility.
    pub fn initial_workspace_scope_for_config(
        auth: &Auth,
        config: &EngineConfig,
    ) -> WorkspaceScope {
        if !auth.workos_enabled()
                ...
                .is_none_or(|token| token.trim().is_empty())
        {
            ...
            Self::initial_workspace_scope(auth)
        }
    }
        ...
        match scope {
            ...
            WorkspaceScope::Development => {
                ...
                let org_id = dev_token_org
                    ...
                    .unwrap_or_else(|| env_or("KEEL_ORG_ID", DEFAULT_ORG_ID));
                let user_id = auth
                    .user_id()
                    .unwrap_or_else(|| env_or("KEEL_USER_ID", DEFAULT_USER_ID));
    ...
    async fn assemble_runtime_inner(
        ...
        let edge_enabled = match profile.scope() {
            ...
            // KEEL_WORKOS_CLIENT_ID="". Only an explicitly configured,
        ...
        let core = match lock {
            Some(lock) => EngineCore::assemble_with_profile_locked(
                profile,
                Arc::new(default_registry_for_data_dir(&config.data_dir)),
        ...
        // Keel has no release service or signed update channel. Do not reuse
        // the inherited updater even if an Avid auto-update env var is set.
        tracing::info!(device_id = %core.device_id, "engine core assembled");

        let host_relay = edge.as_ref().map(|edge| {
            let links = keel_rpc::LinkCache::new(keel_rpc::LinkCacheConfig::new(
                edge.url.clone(),
                Arc::new(auth.clone()),
            ));
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let workspace_scope = Self::initial_workspace_scope_for_config(&auth, &config);
...
pub async fn serve_ipc(
    port: u16,
    service: std::sync::Arc<dyn keel_rpc::RpcService>,
) -> std::io::Result<tokio::task::JoinHandle<()>> {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
    tracing::info!(port, "IPC server listening");
    Ok(tokio::spawn(keel_rpc::serve_ws_listener(listener, service)))
}
...
/// `keel login` persisted, never wait on a prompt nobody can see.
    ...
    loop {
        ...
        match state {
            ...
            AuthState::NeedsOrganization { user } => {
                if !interactive {
                    ...
                    return Err(EngineError::Other(format!(
                        "signed in as {} but no workspace is selected — run `keel login` on this machine to pick one",
                        user.email
                    )));
                }
                if org_reader.is_none() {
                    // Workspace onboarding on the TTY (old keel's
            ...
            AuthState::SignedOut => {
                if !interactive {
                    return Err(EngineError::Other(
                        "not signed in — run `keel login` on this machine first".into(),
                    ));
                }
                if stdin_reader.is_none() {
                    let url = auth.start_headless_sign_in();
                    println!("Sign in to Keel:\n\n  {url}\n");
...
/// TTY workspace onboarding for an org-less session (ports old keel's
...
async fn run_org_onboarding(auth: Auth) {
    let orgs = match auth.list_orgs().await {
        ...
        Err(err) => {
            println!(
                "Could not list workspaces ({err}) — create or select one from the Keel UI to continue."
            );
            return;
        }
    };
...
fn local_device_name(device_id: &str) -> String {
    select_local_device_name(
        [
            std::env::var("KEEL_DEVICE_NAME").ok(),
