...
//!   session id as `RunRequest.resume` (engine-owned, keel sessions.ts:736);
//! - a kill -9 style crash recovers the session id from the run journal
//!   (keel recoverDraft, sessions.ts:538-552) and stamps streaming entries
//!   `aborted`;
...
async fn persistent_session_serves_multiple_turns_on_one_child() {
    ...
    // The session PARKS (keel runsBySession): the second message routes into
...
async fn fresh_crash_auto_resumes_and_notes_the_interruption() {
    ...
    // The run is PICKED BACK UP without any user action (keel: "not just
...
/// Run with: `cargo test -p keel-engine --test restart_resume -- --ignored`
