...
use keel_engine::{AuthState, Engine, EngineConfig, EngineInfo, HarnessId, WorkspaceScope};
use keel_rpc::{connect_ws, memory_client, methods};
...
#[tokio::test]
async fn unconfigured_keel_uses_local_profile_and_explicit_bearer_uses_development() {
    let dir = tempfile::tempdir().unwrap();
    let local = config(dir.path(), "http://127.0.0.1:1".into(), None, None);
    let auth = Engine::build_auth(&local).await;
    let scope = Engine::initial_workspace_scope_for_config(&auth, &local);
    assert_eq!(scope, WorkspaceScope::Local);
    let profile = Engine::resolve_profile(&local, &auth, scope)
        .unwrap()
        .unwrap();
    assert_eq!(profile.store_root(), dir.path().join("profiles/local"));

    let explicit_dev = config(
        dir.path(),
        "http://127.0.0.1:1".into(),
        None,
        Some("developer@team"),
    );
    let auth = Engine::build_auth(&explicit_dev).await;
    assert_eq!(
        Engine::initial_workspace_scope_for_config(&auth, &explicit_dev),
...
#[tokio::test]
async fn transient_refresh_failure_keeps_synced_peer_recovery_without_inherited_updater() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(
        dir.path().join("session.json"),
    ...
    assert!(
        runtime.core().updater().is_none(),
        "Keel has no release channel and must not start the inherited updater"
    );
    runtime.shutdown().await;
}
