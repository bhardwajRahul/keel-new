...
use keel_engine::{EngineCore, HarnessRegistry, RunJournal};
use keel_harness::mock::MockHarness;
use keel_harness::{Harness, HarnessError, RunControls};
use keel_proto::{
...
async fn steer_with_no_live_run_falls_back_to_new_turn() {
    ...
    // dispatch-as-next-turn, per keel's executor.
    queue_as_viewer(
        handle.doc(),
        "cmd-steer-1",
...
async fn processed_commands_are_skipped_on_redelivery() {
    ...
    let verdict = keel_doc::evaluate_command(
        entry,
        &keel_doc::EvaluationContext {
    ...
    assert_eq!(verdict, keel_doc::CommandDisposition::Skip);
}
...
async fn rpc_surface_over_in_memory_transport() {
    ...
    let client = keel_rpc::memory_client(core.rpc_service());

    // ListHarnesses + ListModels.
    let harnesses = client
        .call(keel_rpc::methods::LIST_HARNESSES, serde_json::Value::Null)
        .await
        .unwrap();
    assert_eq!(harnesses[0]["id"], "mock");
    let models = client
        .call(
            keel_rpc::methods::LIST_MODELS,
    ...
    let mut sessions_stream = client
        .subscribe(keel_rpc::methods::WATCH_SESSIONS, serde_json::Value::Null)
        .await
        .unwrap();
    ...
    let mut messages_stream = client
        .subscribe(
            keel_rpc::methods::WATCH_DOC_MESSAGES,
    ...
    let queued = client
        .call(
            keel_rpc::methods::QUEUE_COMMAND,
    ...
    let settled = loop {
        let item = tokio::time::timeout_at(deadline, messages_stream.recv())
            ...
            .expect("stream alive");
        let frame: keel_doc::TranscriptFrame = serde_json::from_value(item).unwrap();
        keel_doc::apply_transcript_frame(&mut materialized, frame).unwrap();
...
async fn respond_input_resolves_pending_question() {
    ...
    impl Harness for AskingHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let answers = (controls.request_input)(vec![keel_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
    ...
    queue_as_viewer(
        ...
        SessionCommandPayload::RespondInput {
            request_id,
            answers: vec![keel_proto::UserInputAnswer {
                question_id: "q1".into(),
                labels: vec!["b".into()],
            }],
        },
        ...
        async fn run(
            ...
            if request.prompt == "second run" {
                ...
                tokio::spawn(async move {
                    ...
                    let _ = (controls.request_input)(vec![keel_proto::UserInputQuestion {
                        id: "q1".into(),
                        header: "Pick".into(),
                        question: "Which one?".into(),
                        options: vec!["a".into(), "b".into()],
...
async fn harness_emitted_input_twin_is_dropped_and_answer_resumes() {
    ...
    impl Harness for DoubleEmitHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let question = keel_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
                    question: "Which one?".into(),
                    options: vec!["a".into(), "b".into()],
...
async fn attachment_upload_then_run_threads_refs_and_paths() {
    ...
    let committed = client
        .call(
            keel_rpc::methods::UPLOAD_COMMIT,
    ...
    // Run with the keel `withAttachments` transport: refs embedded in the
    ...
    let chunk = client
        .call(
            keel_rpc::methods::READ_ATTACHMENT_CHUNK,
...
/// Run with: `cargo test -p keel-engine --test e2e -- --ignored`
#[tokio::test(flavor = "multi_thread")]
...
async fn real_claude_sees_uploaded_image_inline() {
    ...
    let core = EngineCore::assemble(
        &dir,
        Arc::new(keel_engine::default_registry()),
