...
/// The floating-menu surface (keel `.glass-surface` + `menuSurface`):
...
/// One menu row (keel `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (keel's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (keel
...
/// Hairline divider between menu sections (keel `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (keel
...
/// A bordered trailing menu section (keel picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (keel dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (keel dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (keel:
