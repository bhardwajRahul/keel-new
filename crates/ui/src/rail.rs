...
use keel_doc::{MessagePart, MessageRole, SessionMessageEntry};
...
/// `KEEL_SCROLL_TRACE=1` logs per-frame glide positions at `warn` level —
/// the smoothness measurement knob (same family as `KEEL_FRAME_STATS`).
fn scroll_trace_enabled() -> bool {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ENABLED
        .get_or_init(|| std::env::var("KEEL_SCROLL_TRACE").is_ok_and(|v| !v.is_empty() && v != "0"))
}
