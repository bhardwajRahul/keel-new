...
//! (keel parity: localStorage `keel.composer.defaults:v1`, defaults.ts).
//!
...
use keel_proto::{HarnessId, ReasoningLevel};

const FILE_NAME: &str = "composer-defaults.json";

/// Remembered model per harness — id plus display label, mirroring keel's
...
pub struct ComposerDefaults {
    ...
    /// Last reasoning level picked (global, like keel's `reasoning` key).
...
impl ComposerDefaults {
    ...
    /// Remember a pick (keel `saveDefaults({ harness, modelByHarness })`).
