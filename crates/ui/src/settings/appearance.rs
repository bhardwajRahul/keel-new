...
fn miniature_split(accent: AccentPalette) -> AnyElement {
    div()
        .size_full()
        .flex()
        .flex_row()
        .child(div().w_1_2().h_full().overflow_hidden().child(miniature(
            &Theme::for_appearance_with_accent(Appearance::Light, accent),
            Corners::Left,
        )))
        .child(div().w_1_2().h_full().overflow_hidden().child(miniature(
            &Theme::for_appearance_with_accent(Appearance::Dark, accent),
            Corners::Right,
        )))
        .into_any_element()
}
...
fn preview(mode: AppearanceMode, accent: AccentPalette) -> AnyElement {
    match mode {
...
fn helper(mode: AppearanceMode, system: Appearance) -> SharedString {
    match mode {
        ...
        AppearanceMode::System => {
            ...
            format!(
                "Following the system appearance — currently {resolved}. Keel switches with \
...
impl Render for AppearancePage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let selected_accent = appearance::accent(cx);
        let system = cx
        ...
        let colors = AccentPalette::ALL.into_iter().map(|palette| {
            let selected = palette == selected_accent;
            let swatch = Theme::for_appearance_with_accent(theme.appearance, palette).accent;
            div()
                .id(SharedString::from(format!("accent-{}", palette.label())))
                .min_w(px(108.0))
                .h(px(42.0))
                .px(px(10.0))
                .flex()
                ...
                .rounded(px(8.0))
                .border_1()
                .border_color(if selected { swatch } else { theme.border })
                .bg(if selected {
                ...
                .cursor_pointer()
                .child(div().size(px(18.0)).rounded_full().bg(swatch))
                .child(
                    div()
                        .text_size(px(12.0))
                ...
                .on_click(cx.listener(move |_, _, _, cx| {
                    appearance::set_accent(palette, cx);
                    cx.notify();
                }))
        });
                ...
                widgets::page_column()
                    ...
                    .child(
                        widgets::page_subtitle(
                            &theme,
                            "Choose a look for Keel. These settings stay on this device.",
                        )
                        .max_w(px(512.0))
                        .line_height(px(20.0)),
                    )
                    ...
                    .child(
                        div()
                            ...
                            .flex_col()
                            .gap(px(12.0))
                            .child(widgets::field_label(&theme, "Accent color"))
                            .child(div().flex().flex_wrap().gap(px(10.0)).children(colors)),
                    ),
            )
    }
}
