//! Animation kit — the keel motion catalog as reusable helpers over gpui
...
//! - `keel-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// keel's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; keel never does).
...
/// Keel loader pulse period: 2.4s.
pub const KEEL_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
/// Gradient matrix spinner wave period: 750ms.
pub const GRADIENT_SPIN: MotionSpec = MotionSpec::new(750, EASE);
/// One brief pixel/clear cycle around a composer edit or send.
pub const MESSAGE_PIXEL: MotionSpec = MotionSpec::new(240, EASE);
pub const MESSAGE_PIXEL_ACTIVE_MS: u64 = 480;

/// Pixel phases use a fixed clock, so repeated keystrokes extend the effect
/// without pinning it to the first frame of every edit.
pub fn message_pixelated(elapsed_ms: u64) -> bool {
    (elapsed_ms / 120) % 2 == 0
}
...
/// A locally sent message enters in place, with only a small paint-layer lift.
pub fn message_send_in<E>(id: impl Into<ElementId>, element: E) -> AnimationElement<E>
where
    E: Styled + IntoElement + 'static,
{
...
/// (keel also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Keel-pulse floor opacity.
// The loader constants and math live in `keel_proto::motion` (pure phase
// functions); this crate animates them with gpui.
pub use keel_proto::motion::{
...
// the pointer enters. The original keel puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`KEEL_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `KEEL_MOTION_SCALE=10` slows the
...
mod tests {
    ...
    #[test]
    fn message_pixel_phases_alternate() {
        assert!(message_pixelated(0));
        assert!(message_pixelated(119));
        assert!(!message_pixelated(120));
        assert!(!message_pixelated(239));
        assert!(message_pixelated(240));
        assert!(!message_pixelated(360));
    }
    ...
    fn catalog_timings_match_keel() {
        ...
        assert_eq!(KEEL_PULSE.duration_ms, 2400);
