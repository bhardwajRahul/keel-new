...
//!   the banner is attributed to Keel with its icon. The API is deprecated
...
//!   center — those adopt the installed Keel app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Keel icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Keel installed fall back to
...
//! - `KEEL_DISABLE_NOTIFICATIONS` env kill-switch + the
...
const DISABLE_ENV: &str = "KEEL_DISABLE_NOTIFICATIONS";
...
#[cfg(target_os = "macos")]
const MACOS_BUNDLE_ID: &std::ffi::CStr = c"app.keel.local";
...
mod delegate {
    ...
    pub(super) fn always_present() -> *mut Object {
        ...
        *DELEGATE.get_or_init(|| unsafe {
            let mut decl = ClassDecl::new("KeelNotifyDelegate", class!(NSObject))
                .expect("KeelNotifyDelegate registered twice");
            decl.add_method(
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Keel app's id for
