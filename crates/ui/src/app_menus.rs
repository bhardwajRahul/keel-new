...
//!
//! keel never called `cx.set_menus`, so on macOS `NSApp.mainMenu` stayed nil:
...
pub fn init(cx: &mut App) {
    ...
    // Window verbs route to the active window. keel is single-window, so a
...
/// ⌘Q / "Quit Keel". `cx.quit()` runs the platform's standard quit routine,
...
/// The keel menu bar. macOS renders this natively; mac-only entries are gated
...
pub fn app_menus() -> Vec<Menu> {
    ...
    let mut app_items = vec![
        ...
        MenuItem::action("About Keel", About).disabled(true),
        MenuItem::separator(),
    ];
    if macos {
        app_items.extend([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide Keel", Hide),
            MenuItem::action("Hide Others", HideOthers),
            MenuItem::action("Show All", ShowAll),
            MenuItem::separator(),
        ]);
    }
    app_items.push(MenuItem::action("Quit Keel", Quit));

    let mut menus = vec![
        Menu::new("Keel").items(app_items),
