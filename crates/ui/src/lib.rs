//! keel-ui — the gpui viewport. Shell, sidebar, conversation, composer, terminal,
//! diff pane.
//!
...
//! - [`motion`] — the keel animation catalog over gpui `Animation` + cubic-bezier;
...
/// root view, with a compact boot state until the local engine reports ready.
...
fn open_main_window(state: gpui::Entity<state::AppState>, boot: EngineBootConfig, cx: &mut App) {
    // keel window geometry: 1320×880, min 900×600 (feature-inventory §1.1).
    ...
    cx.open_window(
        WindowOptions {
            ...
            app_id: Some("app.keel.local".into()),
            ..Default::default()
        },
        move |window, cx| {
