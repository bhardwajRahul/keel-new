...
//! collapse flags (keel persisted the same set in localStorage).
//!
...
pub struct UiSettings {
    ...
    /// Session notification chimes (done / awaiting-input). `KEEL_DISABLE_SOUND`
    /// overrides.
    pub sound_enabled: bool,
    /// Desktop banner notifications on the same transitions.
    /// `KEEL_DISABLE_NOTIFICATIONS` overrides.
    pub notifications_enabled: bool,
    /// Suppress the banner while a Keel window is focused (the chime covers
    /// the foreground case).
    pub notifications_background_only: bool,
    pub right_pane_width: f32,
    /// Legacy: panel *open* flags are session-scoped in-memory state now
    /// (`shell::SessionPanels`, keel `sessionPanels` parity). Kept for file
    ...
    /// First-run Jev choice. The credential itself is never stored in settings.
    pub jev_onboarding_complete: bool,
}
...
impl ShortcutId {
    ...
    /// Row label (keel lib/shortcuts.ts `SHORTCUT_DEFINITIONS`, verbatim).
