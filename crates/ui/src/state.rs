...
//! port, mirroring keel: if an engine is listening it connects over WebSocket
...
use keel_doc::{SessionMessageEntry, TranscriptDesync, TranscriptFrame};
use keel_engine::{Engine, EngineConfig, EngineRuntime, InstanceLock, rpc::AuthRpc};
use keel_proto::{
    AuthState, Chat, ChatIndicator, DecisionActivity, Device, EngineInfo, HarnessId, Session,
    Space, WorkspaceScope,
};
use keel_rpc::{RpcClient, RpcError, RpcReply, RpcService, connect_ws, memory_client, methods};
...
pub struct EngineBootConfig {
    /// Data directory for the embedded engine (`~/.keel`).
...
impl EngineHandle {
    ...
    pub async fn bootstrap(config: EngineBootConfig) -> anyhow::Result<EngineHandle> {
        ...
        if let Some(handle) = Self::attach_to_daemon(&config).await {
            return Ok(handle);
        }
        ...
        let lock = loop {
            match InstanceLock::acquire(&engine_config.data_dir) {
                ...
                Err(err) => {
                    ...
                    if let Some(handle) = Self::attach_to_daemon(&attach_config).await {
                        return Ok(handle);
                    }
                }
            }
        ...
        let workspace_scope = Engine::initial_workspace_scope_for_config(&auth, &engine_config);
        ...
        let ipc_task = match keel_engine::serve_ipc(engine_config.ipc_port, service).await {
            Ok(task) => Some(task),
            Err(err) => {
                tracing::warn!(
    ...
    async fn attach_to_daemon(config: &EngineBootConfig) -> Option<EngineHandle> {
        ...
        match connect_ws(&url).await {
            Ok(client) => match query_engine_info(&client).await {
                Ok(engine_info) if daemon_matches_boot(config, &engine_info) => {
                ...
                Ok(engine_info) => {
                    tracing::warn!(
                        %url,
                        device_id = %engine_info.device_id,
                        scope = ?engine_info.workspace_scope,
                        "engine identity does not match this local installation; embedding instead"
                    );
                    None
                }
                Err(err) => {
...
/// The fixed IPC port is shared by app versions and data directories. A
/// listener is reusable only when it owns this installation's persisted ID;
/// local-only launches must also reject an older synced runtime on that ID.
fn daemon_matches_boot(config: &EngineBootConfig, info: &EngineInfo) -> bool {
    let expected_id = std::fs::read_to_string(config.data_dir.join("device-id"))
        .ok()
        .map(|id| id.trim().to_owned())
        .filter(|id| !id.is_empty());
    if expected_id.as_deref() != Some(info.device_id.as_str()) {
        return false;
    }
    let local_only = config.workos_client_id.is_none()
        && config
            .edge_token
            .as_deref()
            .is_none_or(|token| token.trim().is_empty());
    !local_only || info.workspace_scope == WorkspaceScope::Local
}
...
pub struct AppState {
    ...
    pub update: Option<keel_update::UpdateStatus>,
...
impl AppState {
    ...
    pub fn apply_chat_config(&mut self, chat_id: &str, config: keel_proto::ChatConfig) {
    ...
    pub fn apply_update(&mut self, status: keel_update::UpdateStatus) {
    ...
    pub fn auth_user(&self) -> Option<&keel_proto::UserProfile> {
    ...
    pub fn apply_transcript_frame(
        ...
        keel_doc::apply_transcript_frame(&mut self.transcript, frame)?;
        if let Some(chat_id) = self.selected_chat.as_deref()
            && let Some(echoes) = self.echoes.get_mut(chat_id)
        {
    ...
    /// Only an engine-reported, fresh selector evaluation may name a decision
    /// backend in the conversation. Sending a prompt alone is not evidence.
    pub fn decision_activity_for(
        &self,
        chat_id: &str,
        now: DateTime<Utc>,
    ) -> Option<DecisionActivity> {
        let session = self.session_for(chat_id)?;
        let age_ms = now
            .signed_duration_since(session.updated_at)
            .num_milliseconds();
        (age_ms <= SESSION_STALE_MS)
            .then_some(session.decision_activity)
            .flatten()
    }
...
mod tests {
    ...
    use keel_engine::{EngineCore, default_registry};
    // `SessionStatus` is only needed to build the fixtures below — the module
    // itself derives everything through `keel_proto::view`.
    use keel_proto::{SessionStatus, UserProfile};
    ...
    #[tokio::test]
    async fn local_viewport_rejects_legacy_synced_daemon() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        let server = tokio::spawn(keel_rpc::serve_ws_listener(
            listener,
        ...
        .await
        .expect("local viewport embeds instead of using legacy synced state");
    ...
    #[tokio::test]
    async fn local_boot_rejects_synced_daemon_even_with_same_device_id() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("device-id"), "same-device").unwrap();
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        let (_state_tx, state_rx) = tokio::sync::watch::channel(DeferredEngineState::Ready);
        let server = tokio::spawn(keel_rpc::serve_ws_listener(
            listener,
        ...
        .await
        .expect("local engine starts without adopting old synced state");
        assert_eq!(handle.mode(), EngineMode::InProcess);
        assert_eq!(handle.engine_info().workspace_scope, WorkspaceScope::Local);
        handle.shutdown().await;
        server.abort();
    ...
    #[tokio::test]
    async fn local_boot_rejects_daemon_for_another_data_dir() {
        let dir = tempfile::tempdir().unwrap();
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        let (_state_tx, state_rx) = tokio::sync::watch::channel(DeferredEngineState::Ready);
        let server = tokio::spawn(keel_rpc::serve_ws_listener(
            ...
            Arc::new(DeferredIdentityRpc {
                engine_info: EngineInfo {
                    device_id: "other-installation".into(),
        ...
        .await
        .expect("separate installation embeds its own engine");
        assert_eq!(handle.mode(), EngineMode::InProcess);
        assert_ne!(handle.engine_info().device_id, "other-installation");
        handle.shutdown().await;
        server.abort();
    }
    ...
    async fn bootstrap_reports_local_assembly_failure_before_returning_a_handle() {
        let dir = tempfile::tempdir().unwrap();
        keel_engine::EngineProfile::local(dir.path()).unwrap();
    ...
    async fn bootstrap_connects_when_daemon_is_listening() {
        // Stand in for `keel headless`: an engine served over the WS IPC port.
        let daemon_dir = tempfile::tempdir().unwrap();
        let core = EngineCore::assemble_with_profile(
            keel_engine::EngineProfile::local(daemon_dir.path()).unwrap(),
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        tokio::spawn(keel_rpc::serve_ws_listener(listener, core.rpc_service()));
    ...
    #[test]
    fn decision_activity_requires_a_fresh_engine_session() {
        let now = Utc::now();
        let mut state = AppState::default();
        assert_eq!(state.decision_activity_for("c", now), None);
        let mut live = session("c", SessionStatus::Idle, 1, now);
        // An ordinary send/Working status does not imply selector activity.
        state.apply_sessions(vec![live.clone()]);
        assert_eq!(state.decision_activity_for("c", now), None);

        live.decision_activity = Some(keel_proto::DecisionActivity {
            backend: keel_proto::DecisionBackend::Laya,
            phase: keel_proto::DecisionPhase::Choosing,
        });
        state.apply_sessions(vec![live.clone()]);
        assert_eq!(
            state.decision_activity_for("c", now),
            live.decision_activity
        );

        live.updated_at = now - TimeDelta::milliseconds(SESSION_STALE_MS + 1);
        state.apply_sessions(vec![live]);
        assert_eq!(state.decision_activity_for("c", now), None);
    }
    ...
    fn apply_chat_config_stamps_the_row() {
        ...
        let config = keel_proto::ChatConfig {
            ...
            reasoning: Some(keel_proto::ReasoningLevel::XHigh),
    ...
    fn project_labels_from_cwd() {
        assert_eq!(project_label(Some("/home/w/dev/keel")), "keel");
        assert_eq!(project_label(Some("/home/w/dev/keel/")), "keel");
    ...
    fn grouped_sidebar_preserves_recency_order() {
        ...
        let chats = [
            chat_with_cwd("a", 9, Some("/dev/keel")),
            chat_with_cwd("b", 8, Some("/dev/zed")),
            chat_with_cwd("c", 7, Some("/dev/keel")),
        ...
        assert_eq!(labels, ["keel", "zed", "No project"]);
        let keel_ids: Vec<&str> = groups[0].chats.iter().map(|c| c.id.as_str()).collect();
        assert_eq!(keel_ids, ["a", "c"]);
        assert!(group_chats(std::iter::empty()).is_empty());
    }

    #[test]
    fn relative_times_match_keel_format() {
        let now = Utc::now();
    ...
    fn chat_location_joins_project_and_branch() {
        ...
        c.branch = Some("keel/rebalance".into());
        assert_eq!(
            chat_location(&c).as_deref(),
            Some("soccertcg · keel/rebalance")
        );
        c.branch = None;
