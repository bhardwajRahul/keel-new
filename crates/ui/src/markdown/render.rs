...
use keel_syntax::{HighlightKind, HighlightSpan, HighlightedDocument};
...
/// Gap between markdown blocks inside one message (keel mdBlockGap).
pub const MD_BLOCK_GAP: f32 = 12.0;
/// Body text size / line height (keel: 14px / 22px).
...
// Table metrics — a port of mugen-markdown 0.6.2's `TableBlock` under keel's
...
/// Uniform cell padding in px (keel `table.cellPadding`).
pub const TABLE_CELL_PADDING: f32 = 12.0;
/// Hairline between rows in px (keel `table.gap`).
pub const TABLE_DIVIDER: f32 = 1.0;
/// Header row font weight (keel `table.headerWeight` = 700).
...
/// Minimum rendered column width in px, padding included (keel
...
/// Hairline tone (keel md theme `table.borderColor`: rgba(255,255,255,0.1)).
...
/// Tight monochrome heading scale (keel: h2 ≈ 16px semibold; headings step
...
/// A GFM table — a port of mugen-markdown's `TableBlock` under keel's md
...
/// keel's `table.headerWeight`; strong runs never drop below semibold).
fn flatten_runs_weighted(runs: &[InlineRun], theme: &Theme, base_weight: FontWeight) -> FlatText {
    ...
    for run in runs {
        ...
        // Links stay monochrome — foreground with an underline (keel's md
...
fn render_code_block(
    ...
    div()
        .rounded(px(10.0))
        // Faint white wash over the near-black panel ≈ #101010 (keel's code
