...
//! `remend`, ported to keel's incremental parser).
//!
...
//! handlers don't apply here — keel renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "keel:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](keel:pending-link)");
        mends("[docs](", "[docs](keel:pending-link)");
        mends("see [do", "see [do](keel:pending-link)");
        mends("![alt](https://x/i.p", "![alt](keel:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](keel:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](keel:pending-link)");
        mends("**a [b", "**a [b](keel:pending-link)**");
    }
