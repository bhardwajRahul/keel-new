...
use crate::theme::{AccentPalette, Appearance, Theme};
...
pub fn init(mode: AppearanceMode, data_dir: impl Into<PathBuf>, cx: &mut App) {
    ...
    let accent = load_accent(&data_dir);
    ...
    Theme::install_with_accent(resolve(mode, system), accent, cx);
}
...
pub fn accent(cx: &App) -> AccentPalette {
    cx.try_global::<AppearanceState>()
        .map(|s| s.accent)
        .unwrap_or_default()
}

/// Update the color immediately in every window and save it for the next launch.
pub fn set_accent(accent: AccentPalette, cx: &mut App) {
    ...
    apply(cx);
    if let Err(err) = save_accent(&data_dir, accent) {
        tracing::warn!(error = %err, "could not persist accent palette");
    }
}

fn save_accent(data_dir: &Path, accent: AccentPalette) -> std::io::Result<()> {
    std::fs::create_dir_all(data_dir)?;
    let path = data_dir.join("accent-palette.json");
    let data = serde_json::to_vec(&accent).map_err(std::io::Error::other)?;
    let tmp = path.with_extension("json.tmp");
    std::fs::write(&tmp, data)?;
    std::fs::rename(tmp, path)
}

fn load_accent(data_dir: &Path) -> AccentPalette {
    std::fs::read(data_dir.join("accent-palette.json"))
        .ok()
...
pub fn apply(cx: &mut App) {
    ...
    let changed = !cx
        .try_global::<Theme>()
        .is_some_and(|t| t.appearance == wanted && t.accent_palette == accent);
    if changed {
        tracing::debug!(?wanted, "appearance: installing palette");
        Theme::install_with_accent(wanted, accent, cx);
        cx.refresh_windows();
    }
...
mod tests {
    ...
    #[test]
    fn accent_persists_and_invalid_file_defaults() {
        let dir = tempfile::tempdir().unwrap();
        assert_eq!(load_accent(dir.path()), AccentPalette::Indigo);
        save_accent(dir.path(), AccentPalette::Teal).unwrap();
        assert_eq!(load_accent(dir.path()), AccentPalette::Teal);
        std::fs::write(dir.path().join("accent-palette.json"), b"bad").unwrap();
        assert_eq!(load_accent(dir.path()), AccentPalette::Indigo);
    }
