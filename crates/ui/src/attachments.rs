...
//!
//! Ports of keel's `composer/use-attachments.ts` (staging/upload),
...
/// Base64 chars per `UploadChunk` (keel state.ts `UPLOAD_CHUNK` — sized for
...
/// `(body_end, refs_start)` byte offsets — the tolerant equivalent of keel's
/// `ATTACHED_IMAGES_RE`.
...
/// the same stuck-offset guard as keel's `readAttachmentImage`).
