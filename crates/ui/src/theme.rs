...
//! lightness steps) into gpui [`Hsla`].
...
/// Accent palette. Semantic colors (danger, warning, success) stay distinct.
...
impl AccentPalette {
    pub const ALL: [Self; 4] = [Self::Indigo, Self::Teal, Self::Rose, Self::Amber];
...
/// Bumped whenever the installed appearance or accent changes.
///
...
fn set_current_accent(accent: AccentPalette) {
    let encoded = accent as u8;
    if CURRENT_ACCENT.swap(encoded, Ordering::Relaxed) != encoded {
        THEME_GENERATION.fetch_add(1, Ordering::Relaxed);
    }
}
...
impl Theme {
    ...
    /// Main-panel header height — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet — the
    ...
    /// (see [`crate::frost::frosted`]). Dark: the reference
    ...
    pub fn dark() -> Self {
        Self {
            ...
            accent_palette: AccentPalette::Indigo,
    ...
    pub fn for_appearance_with_accent(appearance: Appearance, palette: AccentPalette) -> Self {
        let mut theme = Self::for_appearance(appearance);
        theme.accent_palette = palette;
        let (accent, strong) = match (appearance, palette) {
            (_, AccentPalette::Indigo) => return theme,
            (Appearance::Dark, AccentPalette::Teal) => {
                (oklch(0.765, 0.177, 163.223), oklch(0.51, 0.14, 163.223))
            }
            (Appearance::Light, AccentPalette::Teal) => {
                (oklch(0.48, 0.13, 163.225), oklch(0.44, 0.13, 163.225))
            }
            (Appearance::Dark, AccentPalette::Rose) => {
                (oklch(0.718, 0.202, 349.761), oklch(0.52, 0.20, 349.761))
            }
            (Appearance::Light, AccentPalette::Rose) => {
                (oklch(0.49, 0.20, 349.761), oklch(0.45, 0.20, 349.761))
            }
            (Appearance::Dark, AccentPalette::Amber) => {
                (oklch(0.828, 0.189, 84.429), oklch(0.53, 0.14, 68.0))
            }
            (Appearance::Light, AccentPalette::Amber) => {
                (oklch(0.47, 0.12, 68.0), oklch(0.43, 0.12, 68.0))
            }
        };
        theme.accent = accent;
        theme.accent_strong = strong;
        theme.caret = accent;
        theme.selection = accent.opacity(if appearance.is_dark() { 0.35 } else { 0.25 });
        theme
    }
    ...
    pub fn install(appearance: Appearance, cx: &mut App) {
        Self::install_with_accent(appearance, AccentPalette::default(), cx);
    }

    pub fn install_with_accent(appearance: Appearance, accent: AccentPalette, cx: &mut App) {
        set_current_appearance(appearance);
        set_current_accent(accent);
        cx.set_global(Self::for_appearance_with_accent(appearance, accent));
    }
...
mod tests {
    ...
    #[test]
    fn accent_palettes_keep_text_and_button_contrast() {
        for appearance in [Appearance::Dark, Appearance::Light] {
            for palette in AccentPalette::ALL {
                let theme = Theme::for_appearance_with_accent(appearance, palette);
                assert_eq!(theme.accent_palette, palette);
                assert!(
                    contrast_ratio(theme.accent, theme.bg) >= 4.5,
                    "{appearance:?} {palette:?} accent text lacks contrast"
                );
                assert!(
                    contrast_ratio(theme.on_accent, theme.accent_strong) >= 4.0,
                    "{appearance:?} {palette:?} button label lacks contrast"
                );
            }
        }
    }
    ...
    #[test]
    fn switching_accent_invalidates_color_caches() {
        let _guard = lock_appearance();
        set_current_accent(AccentPalette::Indigo);
        let before = theme_generation();
        set_current_accent(AccentPalette::Rose);
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, the app background.
    ...
    #[test]
    fn layout_numbers_match_reference() {
