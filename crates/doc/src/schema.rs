...
//!
//! Part maps: { id, kind: "text"|"tool"|"input"|"error"|"decision", text?: LoroText,
//! call?: json, isError?, questions?: json, resolved?, message?, decision?: json }. Text bodies
//! are **LoroText** so streaming appends RLE-merge (1.03x oplog overhead vs 125x for whole-value
//! rewrites).
...
fn to_doc_part(part: &MessagePart) -> Result<DocPartJson, DocError> {
    Ok(match part {
        ...
        MessagePart::Decision { id, event } => DocPartJson {
            id: id.clone(),
            kind: "decision".into(),
            decision: Some(serde_json::to_value(event.clone().bounded())?),
            ..Default::default()
        },
    })
}
...
fn from_doc_part(p: DocPartJson) -> MessagePart {
    match p.kind.as_str() {
        ...
        "decision" => match p
            .decision
            .and_then(|value| serde_json::from_value::<keel_proto::DecisionEvent>(value).ok())
        {
            Some(event) => MessagePart::Decision {
                id: p.id,
                event: event.bounded(),
            },
            None => MessagePart::Text {
...
///
/// Port of keel's `DocSegmentWriter` diff discipline: called with the *folded* parts of the
...
mod tests {
    ...
    use keel_proto::{AgentEvent, ToolCall};
    ...
    #[test]
    fn decision_receipt_round_trips_as_structured_data_even_when_old() {
        let doc = SessionDoc::init("chat-decision").unwrap();
        let event = keel_proto::DecisionEvent::new(
            "route-1",
            3,
            keel_proto::DecisionBackend::Jev,
            keel_proto::DecisionStage::Intake,
            vec![keel_proto::DecisionCandidate::new(
                "codex",
                "Codex coding loop",
            )],
            keel_proto::DecisionResult::Selected {
                candidate_id: "codex".into(),
            },
            keel_proto::DecisionValidation::Accepted,
            1,
        )
        .with_observed_outcome("dispatch accepted");
        doc.push_message(&SessionMessageEntry {
        ...
        let entries = restored.read_entries().unwrap();
        assert!(matches!(
            &entries[0].parts[0],
            MessagePart::Decision { event: saved, .. } if saved == &event
        ));
    }
