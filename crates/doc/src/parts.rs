...
use keel_proto::{AgentEvent, DecisionEvent, ToolCall, ToolDiff, UserInputQuestion};
...
pub enum MessagePart {
    ...
    /// Host-authored decision receipt. It is a structured observation, never
    /// private model reasoning or generated prose.
    Decision {
        id: String,
        event: DecisionEvent,
    },
...
impl MessagePart {
    ...
    pub fn byte_len(&self) -> usize {
        match self {
            ...
            MessagePart::Decision { event, .. } => {
                serde_json::to_vec(event).map_or(0, |value| value.len())
            }
        }
    }
}
...
///
/// Semantics from keel `foldEventIntoParts`:
...
pub fn fold_event_into_parts(out: &mut Vec<MessagePart>, event: &AgentEvent) {
    match event {
        ...
        AgentEvent::ReasoningDelta { .. } => {
            // Reasoning is not rendered as a transcript part (matches keel).
        }
        AgentEvent::Decision { event } => {
            let event = event.clone().bounded();
            let id = format!("decision-{}", event.id);
            if let Some(existing) = out.iter_mut().find_map(|part| match part {
                MessagePart::Decision { id: part_id, event } if part_id == &id => Some(event),
                _ => None,
            }) {
                *existing = event;
            } else {
                out.push(MessagePart::Decision { id, event });
            }
        }
...
/// continuations (`continuation_id(root, i)`), matching `splitMessageEntry` in keel.
...
mod tests {
    ...
    #[test]
    fn decision_events_fold_as_bounded_idempotent_receipts() {
        let first = keel_proto::DecisionEvent::new(
            "step-1",
            2,
            keel_proto::DecisionBackend::Jev,
            keel_proto::DecisionStage::PreStep,
            vec![keel_proto::DecisionCandidate::new("inspect", "Inspect")],
            keel_proto::DecisionResult::Selected {
                candidate_id: "inspect".into(),
            },
            keel_proto::DecisionValidation::Accepted,
            1,
        );
        let completed = first.clone().with_observed_outcome("3 reads completed");
        let mut parts = Vec::new();
        fold_event_into_parts(&mut parts, &AgentEvent::Decision { event: first });
        fold_event_into_parts(&mut parts, &AgentEvent::Decision { event: completed });
        assert_eq!(parts.len(), 1);
        assert!(matches!(
            &parts[0],
            MessagePart::Decision { event, .. }
                if event.observed_outcome.as_deref() == Some("3 reads completed")
        ));
    }
    ...
    fn session_started_resets_accumulator() {
        ...
        fold_event_into_parts(
            ...
            &AgentEvent::SessionStarted {
                harness: keel_proto::HarnessId::Mock,
                model: "m".into(),
                tools: vec![],
                cwd: "/".into(),
                session_id: "s".into(),
