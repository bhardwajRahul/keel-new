//! Workspace doc schema over `loro` — the per-org entity index that replaces keel's
...
//! sets from any device — matching keel's Mutate surface. Presence rides the room's
//! `EphemeralStore` under keys `presence/{deviceId}` (an online timestamp), replacing
//! keel's 15s heartbeat writes so liveness never grows the oplog.
//!
//! Timestamps are stored as epoch millis (the session-doc convention) and surface as
//! `chrono::DateTime<Utc>` through the `keel_proto` entity types.

use chrono::{DateTime, Utc};
use loro::{ExportMode, LoroDoc, LoroMap, LoroValue, ToJson};
use serde::{Deserialize, Serialize};

use keel_proto::{Chat, ChatConfig, DecisionActivity, Device, Session, SessionStatus, Space};
...
impl WorkspaceDoc {
    ...
    /// latest run and the cwd it was created under (keel stored the same pair
    ...
    pub fn upsert_session(&self, session: &Session) -> Result<(), DocError> {
        ...
        match session.decision_activity {
            Some(activity) => row.insert(
                "decisionActivity",
                LoroValue::from(serde_json::to_value(activity)?),
            )?,
            None => row.delete("decisionActivity")?,
        }
        self.doc.commit();
        Ok(())
    }
...
pub(crate) struct RawSession {
    ...
    updated_at: i64,
    #[serde(default)]
    decision_activity: Option<DecisionActivity>,
}
...
impl From<RawSession> for Session {
    fn from(raw: RawSession) -> Self {
        Session {
            ...
            decision_activity: raw.decision_activity,
        }
    }
}
...
mod tests {
    use super::*;
    use keel_proto::{DecisionBackend, DecisionPhase, HarnessId, SandboxLevel};
    ...
    #[test]
    fn decision_activity_round_trips_and_clears() {
        let ws = WorkspaceDoc::new();
        let mut live = session("chat-1", "dev-a", SessionStatus::Working);
        live.decision_activity = Some(DecisionActivity {
            backend: DecisionBackend::Laya,
            phase: DecisionPhase::Choosing,
        });
        ws.upsert_session(&live).unwrap();
        assert_eq!(
            ws.read_sessions().unwrap()[0].decision_activity,
            live.decision_activity
        );

        live.decision_activity = None;
        ws.upsert_session(&live).unwrap();
        assert_eq!(ws.read_sessions().unwrap()[0].decision_activity, None);
    }
