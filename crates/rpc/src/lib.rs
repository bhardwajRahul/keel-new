//! keel-rpc — the typed control plane (UiRpc / ControlRpc) over WebSocket + in-memory
...
//! byte transports), matching the shape of keel's Effect RPC without the Effect runtime:
//!
...
pub mod methods {
    ...
    /// Live sync introspection (`keel sync` / debug surfaces): per-room
