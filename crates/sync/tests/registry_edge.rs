...
//!
//! ```sh
//! KEEL_EDGE_WS=ws://127.0.0.1:27640 cargo test -p keel-sync --test registry_edge -- --ignored
//! ```
...
use keel_doc::RegistryDoc;
use keel_proto::{Chat, Session, SessionStatus};
use keel_sync::RegistryClient;
...
fn edge_url(org: &str, user: &str) -> String {
    let base = std::env::var("KEEL_EDGE_WS")
        .expect("set KEEL_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:27640");
...
#[tokio::test]
#[ignore = "requires a live edge: set KEEL_EDGE_WS (e.g. ws://127.0.0.1:27640)"]
