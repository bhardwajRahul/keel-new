...
/// `SyncStatus` RPC and `keel sync`. Every 2026-08 incident was debugged
...
impl RoomClient {
    ...
    /// Live counters/clocks for this room (SyncStatus RPC / `keel sync`).
        ...
        if crdt == CrdtType::Loro {
            ...
            match &message {
                ProtocolMessage::Ack { status, .. } => {
                    ...
                    // ack made `keel sync` read "acked 0s ago" while the
