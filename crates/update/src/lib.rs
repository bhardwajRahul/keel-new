//! keel-update — release checking and self-update, shared by the engine (the
//! background checker + `ApplyUpdate`), the CLI (`keel update`), and the UI
...
//! - **Managed** (`~/.keel/app/<ver>` + `current` symlink — the curl|sh
...
/// `keel-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("keel-{version}-{os}-{arch}.tar.gz")
}

/// `keel-<ver>-macos-<arch>-app.tar.gz` — the macOS app update payload.
pub fn mac_app_artifact(version: &str) -> String {
    let (_, arch) = platform_key();
    format!("keel-{version}-macos-{arch}-app.tar.gz")
}
...
pub enum InstallKind {
    /// `~/.keel/app/<ver>/keel` behind the `current` symlink
    /// (curl|sh installer / a previous `keel update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".keel").join("app");
...
pub async fn stage_headless(
    ...
    let result = async {
        ...
        if !unpacked.join("keel").is_file() {
            bail!("tarball {file} did not contain a keel binary");
        }
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("keel").exists() => {}
            Err(err) => {
...
/// Restart the installed engine service (the same units `keel daemon` and the
...
pub fn restart_service() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/sh.keel.app")],
        )
    } else {
        run("systemctl", &["--user", "restart", "keel.service"])
    }
}
...
/// Download + unpack the app tarball into `{data_dir}/updates/<ver>/Keel.app`
...
pub async fn stage_mac_app(
    ...
    let staged = dir.join("Keel.app");
    if staged.join("Contents/MacOS/keel").exists() {
        return Ok(staged);
    }
    ...
    if !staged.join("Contents/MacOS/keel").exists() {
        bail!("app tarball {file} did not contain Keel.app");
    }
    Ok(staged)
}
...
/// `KEEL_AUTO_UPDATE=1|true|yes` — headless daemons apply updates themselves.
fn auto_update_enabled() -> bool {
    std::env::var("KEEL_AUTO_UPDATE")
...
/// stream). Managed installs with `KEEL_AUTO_UPDATE` set stage + apply + service
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.keel/app/0.1.1/keel"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.keel/app")
            }
        );
        assert_eq!(
            detect_install_from(
                Path::new("/Applications/Keel.app/Contents/MacOS/keel"),
                Some(Path::new("/Users/u")),
            ),
            InstallKind::MacApp {
                bundle: PathBuf::from("/Applications/Keel.app")
            }
        );
        // A path merely containing `.app` without the bundle layout is not a bundle.
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/keel"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/keel"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("keel-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("keel-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"keel-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["keel-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("keel"), ver).unwrap();
        }
