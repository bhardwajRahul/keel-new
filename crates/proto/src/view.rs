...
/// The app gate (keel's App.tsx phases). Pure.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GatePhase {
    /// Booting / probing — the local engine status is visible during this phase.
    Loading,
...
/// Session-row sub-line, "project · branch" (keel `chatLocation`): the repo
...
/// Per-kind chip label + one-line detail. Labels match keel's `describeTool`
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
    ...
    // Capitalize the first segment only (keel's style).
...
/// read the same on every surface. `keel-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `keel/<name>`
