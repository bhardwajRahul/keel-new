...
//!
//! Set `KEEL_NO_LOGIN_SHELL=1` to disable the snapshot entirely.
...
mod unix {
    ...
    const BEGIN_MARKER: &str = "__KEEL_SHELL_ENV_BEGIN__";
    const END_MARKER: &str = "__KEEL_SHELL_ENV_END__";
    ...
    pub(super) fn capture() -> Option<OsString> {
        if std::env::var_os("KEEL_NO_LOGIN_SHELL").is_some_and(|v| !v.is_empty()) {
            return None;
        }
    ...
    pub(super) fn snapshot_path(shell: &Path, timeout: Duration) -> Option<OsString> {
        // A custom login PATH may omit /usr/bin. Use the system env binary so
        // the snapshot still reports that PATH instead of failing to run env.
        let script = format!("echo {BEGIN_MARKER}; /usr/bin/env; echo {END_MARKER}");
    ...
    fn run_and_capture(shell: &Path, flags: &[&str], script: &str, timeout: Duration) -> Vec<u8> {
        ...
        cmd.args(flags)
            ...
            .env("KEEL_RESOLVING_ENVIRONMENT", "1")
            .env("TERM", "dumb");
...
"#;
        ...
        fn snapshots_path_from_fake_shell() {
            ...
            let shell = fake_shell(
                dir.path(),
                &format!(
                    "#!/bin/sh\nPATH=\"/keel-test/custom/bin:/usr/bin:/bin\"; export PATH\n{RUN_PAYLOAD}"
                ),
            );
            let path = snapshot_path(&shell, Duration::from_secs(10)).unwrap();
            let path = path.to_string_lossy();
            assert!(path.starts_with("/keel-test/custom/bin:"), "got: {path}");
        }
        ...
        fn falls_back_when_interactive_attempt_hangs() {
            ...
            let shell = fake_shell(
                dir.path(),
                &format!(
                    "#!/bin/sh\ncase \" $* \" in *\" -i \"*) sleep 60;; esac\nPATH=\"/keel-test/fallback/bin:/bin\"; export PATH\n{RUN_PAYLOAD}"
                ),
            );
            let start = Instant::now();
            // Preserve the short timeout while allowing a fallback shell to
            // start under a saturated parallel workspace test run.
            let path = snapshot_path(&shell, Duration::from_millis(1500)).unwrap();
            assert!(
                path.to_string_lossy()
                    .starts_with("/keel-test/fallback/bin"),
                "got: {}",
                path.to_string_lossy()
            );
            // First attempt burned ~1.5s then was killed; the whole resolve
