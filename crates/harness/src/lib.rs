//! keel-harness — one interface over coding agents, all driven through the
...
use keel_proto::{
    AgentEvent, CapabilityInventory, HarnessId, Model, ReasoningLevel, RunRequest, SlashCommand,
...
pub struct RunControls {
    /// The run sends questions and awaits answers (blocks the agent, mirrors keel).
...
pub trait Harness: Send + Sync {
    ...
    /// Provider-scoped capabilities observed from its live protocol. Entries
    /// remain advertised until an actual typed execution path confirms them.
    async fn capabilities(&self) -> Result<CapabilityInventory, HarnessError> {
        Ok(CapabilityInventory::empty(self.id()))
    }
...
/// the proper background-crash message old keel showed (user requirement).
