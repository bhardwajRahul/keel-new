...
//! - Permission requests go through the engine's user input bridge
...
//!   bridge, `create_plan` asks for approval, and todos render as a chip.
...
use keel_proto::{
    AgentEvent, CapabilityEffect, CapabilityInventory, CapabilityInvocation, CapabilityKind,
    CapabilityPrecondition, CapabilityScope, CapabilityStatus, DoneStatus, HarnessId, Model,
    ModelOption, ModelOptionChoice, ProviderCapability, ReasoningLevel, RunRequest, SlashCommand,
    SteeringMode, UserInputAnswer, UserInputQuestion,
};

use crate::jsonrpc::{Incoming, RpcClient};
use crate::{Harness, HarnessError, RunControls, Signal, send_signal, shutdown_child};
use normalize::{cursor_todo_events, map_update, parse_commands};
...
fn find_on_paths(exe: &str, extra: Vec<PathBuf>) -> Option<PathBuf> {
    find_on_paths_matching(exe, extra, |_| true)
}

fn find_on_paths_matching(
    exe: &str,
    extra: Vec<PathBuf>,
    ...
    candidates.into_iter().find(|p| p.exists() && usable(p))
}

/// The Hermes installer leaves a small shell launcher in ~/.local/bin. If its
/// checkout was removed, that launcher still exists but cannot start ACP.
fn hermes_launcher_usable(path: &Path) -> bool {
    if !runnable_file(path) {
        return false;
    }
    if std::fs::metadata(path).is_ok_and(|m| m.len() > 4096) {
        return true; // Inspect only small installer shims, never load a native binary.
    }
    let Ok(script) = std::fs::read_to_string(path) else {
        return true; // A native executable is not a shell launcher.
    };
    if !script.starts_with("#!") {
        return true;
    }
    for line in script.lines() {
        let Some(command) = line.trim().strip_prefix("exec ") else {
            continue;
        };
        let target = if let Some(quoted) = command.strip_prefix('"') {
            quoted.split('"').next().unwrap_or_default()
        } else {
            command.split_whitespace().next().unwrap_or_default()
        };
        let target = Path::new(target);
        if target.is_absolute() && !runnable_file(target) {
            return false;
        }
    }
    true
...
fn pi_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        ...
        install_hint: "pi-acp (searched PATH, the login shell's PATH, npm global bins, \
             ...
             when npx is available; the adapter must match the installed pi CLI. \
             Install the current pi CLI with `npm install -g \
        ...
        steering_mode: SteeringMode::TurnBoundary,
        // pi's thinking ladder (minimal→max; its extra "off" tier has no keel
...
// pi-acp 0.0.33 requires pi >= 0.80.4. The older adapter still speaks to the
// installed @mariozechner CLI, and is pinned rather than upgraded at runtime.
fn pi_version_requires_legacy_adapter(version: &str) -> bool {
    let parts = version
        .trim()
    ...
    matches!(parts, Ok(parts) if parts.len() == 3 && parts.as_slice() < [0, 80, 4].as_slice())
}

fn pi_requires_legacy_adapter() -> bool {
    let pi = std::env::var_os("PI_ACP_PI_COMMAND")
        .filter(|path| !path.is_empty())
        .map(PathBuf::from)
        .or_else(|| find_on_paths("pi", npm_global_bins("pi")));
    let Some(pi) = pi else { return false };
    if let Ok(resolved) = std::fs::canonicalize(&pi) {
        if resolved
            .components()
            .any(|part| part.as_os_str() == "@mariozechner")
        {
            return true;
        }
        if resolved
            .components()
            .any(|part| part.as_os_str() == "@earendil-works")
        {
            return false;
        }
    }
    let Ok(output) = std::process::Command::new(pi).arg("--version").output() else {
        return false;
    };
    pi_version_requires_legacy_adapter(&String::from_utf8_lossy(&output.stdout))
}

fn pi_adapter_is_legacy(path: &Path) -> bool {
    let Some(package) = std::fs::canonicalize(path).ok().and_then(|script| {
        script
            .parent()?
            .parent()
    ...
    std::fs::read(package)
        ...
        .is_some_and(|meta| meta["name"] == "pi-acp" && meta["version"] == "0.0.25")
}
...
pub struct AcpHarness {
    ...
    kill_grace: Duration,
    /// Discovery result cache: one protocol snapshot supplies commands and the
    /// broader provider capability inventory.
    capabilities: tokio::sync::OnceCell<CapabilityInventory>,
...
impl AcpHarness {
    ...
    fn resolve_launch(&self) -> Result<(PathBuf, Vec<String>), HarnessError> {
        ...
        if let Some(p) = &self.executable {
            if self.spec.id == HarnessId::Hermes && !hermes_launcher_usable(p) {
            ...
            && !p.is_empty()
        {
            ...
            return Ok((p, spec_args));
        }
        if self.spec.id == HarnessId::Pi && pi_requires_legacy_adapter() {
            if let Some(adapter) = find_on_paths(self.spec.executable, (self.spec.extra_paths)())
                && pi_adapter_is_legacy(&adapter)
            {
                return Ok((adapter, spec_args));
            }
            let npx = find_on_paths("npx", Vec::new())
                .ok_or_else(|| HarnessError::NotInstalled(self.spec.install_hint.into()))?;
            return Ok((npx, vec!["-y".into(), "pi-acp@0.0.25".into()]));
        }
        let candidate = if self.spec.id == HarnessId::Hermes {
            find_on_paths_matching(
                self.spec.executable,
                (self.spec.extra_paths)(),
                hermes_launcher_usable,
            )
        } else {
            find_on_paths(self.spec.executable, (self.spec.extra_paths)())
        };
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "keel_harness::acp", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
    ...
    /// Short-lived discovery run for [`Harness::capabilities`]: initialize,
    /// then try one unauthenticated `session/new`. Best-effort — an agent that
    /// refuses sessions before login still surfaces handshake capabilities.
    async fn discover_capabilities(&self) -> Result<CapabilityInventory, HarnessError> {
        ...
        let discovery = async {
            let init = client
                ...
                .await?;
            let mut inventory = capability_inventory(self.spec.id, &init, None);
            let cwd = std::env::var("HOME").unwrap_or_else(|_| "/".into());
            if let Ok(session) = client
                .request("session/new", json!({ "cwd": cwd, "mcpServers": [] }))
                .await
            {
                inventory = capability_inventory(self.spec.id, &init, Some(&session));
                if !inventory
                    .capabilities
                    .iter()
                    .any(|c| c.kind == CapabilityKind::SlashCommand)
                {
            ...
            Ok::<CapabilityInventory, HarnessError>(inventory)
        };
        ...
        match result {
            ...
            Err(_) => Err(HarnessError::Protocol(
                "capability discovery timed out".into(),
            )),
        }
    }
...
/// Map an advertised `thought_level` value id onto keel's ladder.
...
fn models_from_session(session_response: &Value, catalog: &[Model]) -> Vec<Model> {
    ...
    let ladder: Vec<ReasoningLevel> = config_options
        ...
        .unwrap_or_default();
    // The thought-level select describes the model currently active in this
    // session. It can be narrower than a saved catalog snapshot; applying it
    // to every model would hide valid levels on other model rows.
    let active_model = config_options
        .iter()
    ...
    let build = |id: &str,
     ...
     -> Model {
        ...
        Model {
            ...
            reasoning_levels: if !ladder.is_empty() && active_model == Some(id) {
                ladder.clone()
            } else {
                known
                    .filter(|m| !m.reasoning_levels.is_empty())
                    .map(|m| m.reasoning_levels.clone())
                    .unwrap_or_else(|| ladder.clone())
            },
            options,
        }
    };
...
/// keel's own (forced to the no-prompts choice), model rides the model rows,
...
/// catalogs (keel never declares the boolean config capability, so adapters
...
impl Harness for AcpHarness {
    ...
    fn installed(&self) -> bool {
        if let Some(path) = &self.executable {
            return self.spec.id != HarnessId::Hermes || hermes_launcher_usable(path);
        }
        if let Some(path) = std::env::var_os(self.spec.env_override).filter(|v| !v.is_empty()) {
            return self.spec.id != HarnessId::Hermes || hermes_launcher_usable(Path::new(&path));
        }
        if self.spec.id == HarnessId::Hermes {
            find_on_paths_matching(
                self.spec.cli_executable,
                (self.spec.cli_extra_paths)(),
                hermes_launcher_usable,
            )
            .is_some()
        } else {
    ...
    async fn commands(&self) -> Result<Vec<SlashCommand>, HarnessError> {
        let inventory = self
            .capabilities
            .get_or_try_init(|| self.discover_capabilities())
            .await
            .cloned()?;
        Ok(inventory
            .capabilities
            .into_iter()
            .filter(|item| item.kind == CapabilityKind::SlashCommand)
    ...
    async fn capabilities(&self) -> Result<CapabilityInventory, HarnessError> {
        self.capabilities
            .get_or_try_init(|| self.discover_capabilities())
            .await
            .cloned()
...
fn initialize_params(harness: HarnessId) -> Value {
    ...
    json!({
        ...
        // is what keel wants — the working tree is the source of truth for
...
fn session_scope(session_id: &str) -> CapabilityScope {
...
fn capability_preconditions(scope: &CapabilityScope) -> Vec<CapabilityPrecondition> {
    let mut preconditions = Vec::new();
    if matches!(scope, CapabilityScope::Session { .. }) {
        preconditions.push(CapabilityPrecondition::ActiveSession);
    }
    preconditions.push(CapabilityPrecondition::RevalidateAdvertisement);
    preconditions
}

fn slash_capability(command: SlashCommand, scope: CapabilityScope) -> ProviderCapability {
    ProviderCapability {
        id: format!("slash:{}", command.name),
        kind: CapabilityKind::SlashCommand,
        name: command.name,
        description: command.description,
        preconditions: capability_preconditions(&scope),
        scope,
        // `availableCommands` has no typed execute operation. Keel currently
        // sends `/name` as ordinary prompt text, which is not execution proof.
...
fn add_capability(capabilities: &mut Vec<ProviderCapability>, item: ProviderCapability) {
    if let Some(index) = capabilities
        .iter()
        .position(|existing| existing.id == item.id && existing.kind == item.kind)
    {
        // A session advertisement is more precise than the provider-wide
        // handshake, but never upgrades executable status.
        if matches!(item.scope, CapabilityScope::Session { .. }) {
...
fn named_arrays<'a>(value: &'a Value, keys: &[&str], depth: u8, out: &mut Vec<&'a Value>) {
    if depth == 0 {
        return;
    }
    match value {
        Value::Object(object) => {
            for (key, child) in object {
                ...
                named_arrays(child, keys, depth - 1, out);
            }
        }
        Value::Array(items) => {
            for child in items {
...
fn descriptor_capabilities(
    ...
    named_arrays(value, keys, 4, &mut descriptors);
    descriptors
        .into_iter()
        .filter_map(|descriptor| {
            let name = descriptor
                ...
                .or_else(|| descriptor.get("toolName"))
                .and_then(Value::as_str)?;
            if name.is_empty() {
                return None;
            }
            ...
            let mut preconditions = capability_preconditions(&scope);
            let (invocation, effect) = match kind {
                CapabilityKind::SessionMode => (
                    CapabilityInvocation::ProviderOwned,
                    CapabilityEffect::ChangesSessionMode,
                ),
                CapabilityKind::McpTool | CapabilityKind::ProviderTool => (
                    CapabilityInvocation::ProviderOwned,
                    CapabilityEffect::InvokesProviderTool,
                ),
                CapabilityKind::HostAction => {
                    preconditions.push(CapabilityPrecondition::HostAuthorization);
                    (
                        CapabilityInvocation::HostOwned,
                        CapabilityEffect::HostDefined,
                    )
                }
                _ => (
                    CapabilityInvocation::ProviderOwned,
                    CapabilityEffect::ProviderDefined,
                ),
            };
...
/// Build one honest capability snapshot from ACP wire responses. The protocol
/// can advertise commands, skills, modes, and tools; only a later successful
/// typed operation may promote an entry to `ConfirmedExecutable`.
fn capability_inventory(
    ...
    let mut inventory = CapabilityInventory {
        provider,
        observed_session_id: (!session_id.is_empty()).then(|| session_id.to_owned()),
        capabilities: Vec::new(),
        provider_owned_tools_opaque: true,
    };

    for command in scan_available_commands(initialize) {
        add_capability(
            &mut inventory.capabilities,
            slash_capability(command, CapabilityScope::Provider),
        );
    }
    for item in descriptor_capabilities(
        initialize,
    ...
    .chain(descriptor_capabilities(
        ...
        add_capability(&mut inventory.capabilities, item);
    }
    ...
    for command in scan_available_commands(session) {
        add_capability(
            &mut inventory.capabilities,
            slash_capability(command, scope.clone()),
        );
    }
    for item in descriptor_capabilities(
        session,
    ...
    if let Some(options) = session.get("configOptions").and_then(Value::as_array) {
        for option in options
            .iter()
            .filter(|option| option.get("category").and_then(Value::as_str) == Some("mode"))
        {
                ...
                .flatten()
            {
                ...
                add_capability(
                    ...
                    ProviderCapability {
                        ...
                        kind: CapabilityKind::SessionMode,
                        name: choice
                            .get("name")
                            .and_then(Value::as_str)
                            .unwrap_or(value)
                        ...
                        scope: scope.clone(),
                        status: CapabilityStatus::Advertised,
                        invocation: CapabilityInvocation::SessionConfig {
                            config_id: config_id.to_owned(),
                            value: value.to_owned(),
                        },
                        input_hint: None,
                        preconditions: capability_preconditions(&scope),
                        effect: CapabilityEffect::ChangesSessionMode,
                    },
                );
            }
        }
    ...
    inventory.capabilities.sort_by(|a, b| a.id.cmp(&b.id));
    inventory
}
...
fn config_option_sets(
    ...
    for option in options {
        ...
        let wanted: Option<Value> = match (kind, category) {
            ...
            // Preserve the agent's current permission mode unless the run
            // explicitly selected an advertised mode. A model or decision
            // backend must never silently promote a session to full access.
...
/// Answer a server→client request outside the live input bridge. A permission
/// request cannot acquire authority from a routing decision or unattended
/// default, so this fallback cancels it. The live path asks the user.
fn handle_server_request(
    ...
    match method {
        ...
        // A plan is an approval request. Without a live input bridge it must
        // stay unapproved, while the plan remains visible in the transcript.
        CURSOR_CREATE_PLAN => {
            client.respond(&id, json!({ "outcome": { "outcome": "cancelled" } }));
        ...
        // finished. Image generation has nowhere to land in a keel session.
        CURSOR_TASK => {
        ...
        CURSOR_GENERATE_IMAGE => {
            client.respond(
                &id,
                json!({ "outcome": { "outcome": "rejected", "reason": "keel cannot render generated images" } }),
            );
            Vec::new()
        }
        _ => {
            tracing::debug!(target: "keel_harness::acp", "unhandled server request: {method}");
...
/// A permission request is a user-facing choice when any option lacks an
/// allow/reject kind. Both choices and actual tool permissions use the same
/// input bridge; this distinction only controls the prompt wording.
...
/// The live-run request handler asks the user for both tool permissions and
/// question-shaped choices through the engine's input bridge. A dropped
/// resolver degrades to `cancelled`.
fn handle_server_request_live(
    ...
    if method == CURSOR_CREATE_PLAN {
        ask_cursor_plan(client, id, params, request_input, open_questions);
        return cursor_todo_events(params, CURSOR_PLAN_CHIP);
    }
...
/// one of these gets a response even when keel has nothing to do with it.
...
/// Cursor requires explicit approval of its proposed plan. The plan text is
/// included in the input bridge so the user can review it before choosing.
fn ask_cursor_plan(
    client: &RpcClient,
    id: Value,
    params: &Value,
    ...
    let overview = params.get("overview").and_then(Value::as_str).unwrap_or("");
    let plan = params.get("plan").and_then(Value::as_str).unwrap_or("");
    let question = UserInputQuestion {
        id: new_message_id(),
        header: "Plan approval".into(),
        question: format!("{title}\n{overview}\n\n{plan}"),
        options: vec!["Approve plan".into(), "Reject plan".into()],
        multi_select: false,
    };
    let client = client.clone();
    let request_input = std::sync::Arc::clone(request_input);
    ...
    tokio::spawn(async move {
        ...
        let outcome = match picked {
            Some("Approve plan") => "accepted",
            Some("Reject plan") => "rejected",
            _ => "cancelled",
        };
        client.respond(&id, json!({ "outcome": { "outcome": outcome } }));
        open_questions.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
    });
}
...
/// One `cursor/ask_question` entry: the keel-side question plus what is
/// needed to answer it — the wire id, and the label→optionId table (keel's
    ...
    params
        ...
        .filter_map(|q| {
            ...
            let choices: Vec<(String, String)> = q
                ...
                .collect();
            // An option-less question has no answer keel could send back.
            if choices.is_empty() {
                return None;
            }
            Some(CursorQuestion {
                wire_id,
                question: UserInputQuestion {
                    // Keel-minted: cursor's ids ("q1") repeat across turns.
                    id: new_message_id(),
...
/// unblocks without keel inventing a pick.
...
async fn run_session(session: Session) {
    ...
    let setup = async {
        ...
        // pi-acp accepts mcpServers but does not connect them to pi. Do not
        // advertise a computer-use path that its runtime cannot execute.
        let computer_use = if harness == HarnessId::Pi {
            Vec::new()
        } else {
            native_computer_use::installed_server()
                .into_iter()
                .collect::<Vec<_>>()
        };
        let session_params = json!({ "cwd": request.cwd, "mcpServers": computer_use });
        ...
        //
        // Cursor and Codex advertise some settings for the current model.
        // Set that model first, then apply effort, collaboration, and fast
        ...
        if matches!(harness, HarnessId::Cursor | HarnessId::Codex) {
        ...
        for (config_id, payload) in config_option_sets(
            ...
            if matches!(harness, HarnessId::Cursor | HarnessId::Codex) && config_id == "model" {
                continue;
            }
            let mut params = serde_json::Map::new();
            params.insert("sessionId".into(), session_id.clone().into());
    ...
    let (session_id, steer_ext, init_commands) = tokio::select! {
        res = setup => match res {
            ...
            Err(e) => {
                ...
                tracing::warn!(target: "keel_harness::acp", %error, "agent setup failed");
                let _ = event_tx
    ...
    // `KEEL_ACP_QUIET_SETTLE_MS` overrides; 0 disables.
    let quiet_settle: Option<Duration> = if cost_hint_enabled {
        None
    } else {
        match std::env::var("KEEL_ACP_QUIET_SETTLE_MS")
            .ok()
...
mod tests {
    use super::*;
    use keel_proto::{TodoItem, ToolCall};

    #[test]
    fn pi_adapter_tracks_the_cli_protocol_generation() {
        assert!(pi_version_requires_legacy_adapter("0.55.3\n"));
        assert!(pi_version_requires_legacy_adapter("v0.80.3"));
        assert!(!pi_version_requires_legacy_adapter("0.80.4"));
        assert!(!pi_version_requires_legacy_adapter("0.87.1"));
        assert!(!pi_version_requires_legacy_adapter("unknown"));

        let package = tempfile::tempdir().unwrap();
        let bin = package.path().join("dist/index.js");
        std::fs::create_dir_all(bin.parent().unwrap()).unwrap();
        std::fs::write(&bin, "").unwrap();
        std::fs::write(
            package.path().join("package.json"),
            r#"{"name":"pi-acp","version":"0.0.25"}"#,
        )
        .unwrap();
        assert!(pi_adapter_is_legacy(&bin));
        std::fs::write(
            package.path().join("package.json"),
            r#"{"name":"pi-acp","version":"0.0.33"}"#,
        )
        .unwrap();
        assert!(!pi_adapter_is_legacy(&bin));
    }
    ...
    #[test]
    fn hermes_stale_installer_launcher_is_not_usable() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("venv/bin/hermes");
        let launcher = dir.path().join("hermes");
        std::fs::write(
            &launcher,
            format!(
                "#!/usr/bin/env bash\nexec \"{}\" \"$@\"\n",
        ...
        assert!(!hermes_launcher_usable(&launcher));
        let harness = AcpHarness::hermes().with_executable(&launcher);
        assert!(!harness.installed());
        assert!(matches!(
            harness.launch_program(),
        ...
        assert!(hermes_launcher_usable(&launcher));
        assert!(harness.installed());
        assert_eq!(harness.launch_program().unwrap(), launcher);
    }
    ...
    fn models_prefer_the_model_config_option_over_legacy_available_models() {
        ...
        // The active model's live ladder wins over a stale catalog snapshot;
        // other model rows retain their own catalog ladder.
    ...
    /// keyed by cursor's wire ids, not the labels keel showed the user.
    #[test]
    fn cursor_questions_round_trip_labels_back_to_option_ids() {
        ...
        // Cursor's repeatable ids never leak into keel's question ids.
    ...
    fn cursor_todo_notifications_map_to_stable_chips() {
        ...
        // Everything else is noise keel has nothing to render for.
    ...
    fn codex_exec_approval_options_are_not_a_question() {
        ...
        // session allow + a prefix-rule amendment). This is a permission,
    ...
    #[test]
    fn mode_config_option_requires_explicit_choice() {
        let codex = json!({
            "sessionId": "s-1",
            "configOptions": [{
                "id": "mode",
        ...
        explicit.insert("mode".into(), json!("agent-full-access"));
        assert_eq!(
            config_option_sets(&codex, None, &[], &explicit),
    ...
    #[test]
    fn capability_snapshot_keeps_provider_owned_entries_advertised_and_opaque() {
        let initialize = json!({
            "agentCapabilities": {
                "availableCommands": [{
                    "name": "compact",
                ...
                "availableSkills": [{ "name": "review", "description": "Review code" }],
                "mcpTools": [{ "name": "browser_click", "description": "Click a target" }]
            }
        });
        let session = json!({
            ...
            "availableTools": [{ "name": "terminal_exec", "description": "Run a command" }]
        });

        let inventory = capability_inventory(HarnessId::Codex, &initialize, Some(&session));
        assert_eq!(
            inventory.observed_session_id.as_deref(),
            Some("probe-session")
        );
        assert!(inventory.provider_owned_tools_opaque);
        assert_eq!(inventory.capabilities.len(), 5, "{inventory:?}");
        assert!(
            inventory
                .capabilities
                .iter()
                .all(|item| item.status == CapabilityStatus::Advertised)
        );

        let slash = inventory
            .capabilities
            .iter()
            .find(|item| item.id == "slash:compact")
            .unwrap();
        assert_eq!(slash.invocation, CapabilityInvocation::PromptText);
        assert_eq!(slash.input_hint.as_deref(), Some("instructions"));

        let mode = inventory
            .capabilities
            .iter()
            .find(|item| item.id == "mode:plan")
            .unwrap();
        assert_eq!(
            mode.invocation,
            CapabilityInvocation::SessionConfig {
                config_id: "mode".into(),
                value: "plan".into()
            }
        );
        assert!(matches!(mode.scope, CapabilityScope::Session { .. }));

        let tool = inventory
            .capabilities
            .iter()
            .find(|item| item.id == "provider-tool:terminal_exec")
            .unwrap();
        assert_eq!(tool.invocation, CapabilityInvocation::ProviderOwned);
        assert_eq!(tool.effect, CapabilityEffect::InvokesProviderTool);
    }
}
