...
use keel_proto::{AgentEvent, SlashCommand, TodoItem, ToolCall, ToolDiff};
...
/// content) to the typed [`ToolCall`] keel renders. Best-effort: agents vary
...
pub(crate) fn map_update(update: &Value) -> Vec<AgentEvent> {
    ...
    match kind {
        ...
        "plan" => {
            ...
            vec![
                AgentEvent::ToolCall {
                    id: keel_proto::LIVE_PLAN_TOOL_ID.into(),
        ...
        // Context-window gauge, not per-turn input/output tokens — keel's
