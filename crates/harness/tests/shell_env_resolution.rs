...
async fn cli_on_login_shell_path_only_is_resolved() {
    ...
    // A current Pi version uses the current adapter found on the login PATH.
    ...
    unsafe {
        ...
        std::env::remove_var("KEEL_NO_LOGIN_SHELL");
    }

    let snapshot = keel_harness::shell_env::login_shell_path().expect("snapshot captured");
