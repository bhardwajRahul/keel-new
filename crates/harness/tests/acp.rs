...
async fn happy_path_maps_chunks_tools_diffs_plans_and_commands() {
    ...
    assert!(events.contains(&AgentEvent::ToolCall {
        id: "t1".into(),
        call: ToolCall::Exec {
            command: "cargo test -p keel-harness".into()
        },
    }));
    ...
    assert!(exec_output.starts_with("   Compiling keel-harness"));
...
async fn config_options_apply_requested_model_and_effort() {
    ...
    req.reasoning = Some(keel_proto::ReasoningLevel::Medium);
...
async fn ultrathink_prefixes_the_prompt_for_claude() {
    ...
    req.reasoning = Some(keel_proto::ReasoningLevel::Ultrathink);
...
#[tokio::test]
async fn permission_requests_require_an_explicit_user_choice() {
    let (steer_tx, steer_rx) = mpsc::channel(8);
    let token = CancellationToken::new();
    let controls = RunControls {
        request_input: Box::new(move |questions| {
    ...
    // auto_approve is true in request(), but authority still comes from this
    // explicit input-bridge choice.
    assert!(
        events.contains(&AgentEvent::TextDelta {
            text: "approved".into()
        }),
...
#[tokio::test]
async fn unanswered_permission_requests_cancel_safely() {
    let (steer_tx, steer_rx) = mpsc::channel(8);
    let token = CancellationToken::new();
    let controls = RunControls {
    ...
    let _keep = (steer_tx, token);
    let events = run_to_end(&harness(), request("scenario:permission-cancel"), controls).await;
    assert!(events.contains(&AgentEvent::TextDelta {
        text: "cancelled safely".into()
    }));
    assert_eq!(dones(&events), vec![(DoneStatus::Completed, None)]);
}

#[tokio::test]
async fn cursor_plan_requires_explicit_approval() {
    let (steer_tx, steer_rx) = mpsc::channel(8);
    let token = CancellationToken::new();
    let controls = RunControls {
        request_input: Box::new(move |questions| {
    ...
    let _keep = (steer_tx, token);
    let events = run_to_end(&harness(), request("scenario:cursor-plan"), controls).await;
    assert!(
        events.contains(&AgentEvent::TextDelta {
...
/// `cargo test -p keel-harness --test acp -- --ignored real_discovery`
#[tokio::test]
...
async fn real_discovery_yields_base_models_with_traits() {
    ...
    for m in &claude {
        ...
        assert!(
            m.reasoning_levels
                .contains(&keel_proto::ReasoningLevel::Ultrathink),
...
/// `cargo test -p keel-harness --test acp -- --ignored real_claude`
#[tokio::test]
...
/// `cargo test -p keel-harness --test acp -- --ignored real_cursor`
#[tokio::test]
...
/// `cargo test -p keel-harness --test acp -- --ignored real_cursor_todos`
#[tokio::test]
...
fn descriptor_surface_matches_registry_expectations() {
    ...
    assert_eq!(
        ...
        &[
            keel_proto::ReasoningLevel::Low,
            keel_proto::ReasoningLevel::Medium,
            keel_proto::ReasoningLevel::High,
        ]
    );
}
...
fn hermes_and_pi_descriptor_surfaces_match_registry_expectations() {
    ...
    assert_eq!(
        pi.reasoning_levels(),
        &[
            keel_proto::ReasoningLevel::Minimal,
            keel_proto::ReasoningLevel::Low,
            keel_proto::ReasoningLevel::Medium,
            keel_proto::ReasoningLevel::High,
            keel_proto::ReasoningLevel::XHigh,
            keel_proto::ReasoningLevel::Max,
        ]
    );
}
...
/// `cargo test -p keel-harness --test acp -- --ignored real_claude_starve`
#[tokio::test]
...
/// `cargo test -p keel-harness --test acp -- --ignored --nocapture real_all_harnesses`
#[tokio::test]
...
/// `cargo test -p keel-harness --test acp -- --ignored --nocapture real_claude_debug`
#[tokio::test]
