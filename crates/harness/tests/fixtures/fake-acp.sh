#!/bin/sh
# Fake ACP agent for keel-harness tests.
#
...
if has "$line" '"method":"session/load"'; then
  ...
  # The harness may attach the user's installed computer-use MCP server.
  # Its own unit tests validate that descriptor; this fixture only requires
  # the ACP session field to be present.
...
*scenario:happy*)
  ...
  update '{"sessionUpdate":"tool_call","toolCallId":"t1","title":"cargo test -p keel-harness","kind":"execute","status":"pending","rawInput":{"command":"cargo test -p keel-harness"}}'
  update '{"sessionUpdate":"tool_call_update","toolCallId":"t1","status":"completed","content":[{"type":"content","content":{"type":"text","text":"   Compiling keel-harness v0.1.21\n    Finished `dev` profile [unoptimized] in 2.41s\n     Running tests/acp.rs\n\nrunning 13 tests\ntest result: ok. 13 passed; 0 failed; 0 ignored"}}]}'
...
*scenario:permission-cancel*)
  emit "{\"id\":78,\"method\":\"session/request_permission\",\"params\":{\"sessionId\":\"$SID\",\"toolCall\":{\"toolCallId\":\"t2\"},\"options\":[{\"optionId\":\"once\",\"name\":\"Allow once\",\"kind\":\"allow_once\"},{\"optionId\":\"no\",\"name\":\"Reject\",\"kind\":\"reject_once\"}]}}"
  read -r ans || exit 1
  { has "$ans" '"id":78' && has "$ans" '"outcome":"cancelled"'; } ||
    { emit "{\"id\":$pid,\"result\":{\"stopReason\":\"refusal\"}}"; exit 0; }
  update '{"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"cancelled safely"}}'
  emit "{\"id\":$pid,\"result\":{\"stopReason\":\"end_turn\"}}"
  ;;
...
*scenario:cursor-plan*)
  emit "{\"id\":79,\"method\":\"cursor/create_plan\",\"params\":{\"sessionId\":\"$SID\",\"name\":\"Implementation plan\",\"overview\":\"Review before execution\",\"plan\":\"1. Read\\n2. Edit\",\"todos\":[{\"content\":\"Read\",\"status\":\"pending\"}]}}"
  read -r ans || exit 1
  { has "$ans" '"id":79' && has "$ans" '"outcome":"accepted"'; } ||
    { emit "{\"id\":$pid,\"result\":{\"stopReason\":\"refusal\"}}"; exit 0; }
...
*scenario:quiet-starve*)
  ...
  # KEEL_ACP_QUIET_SETTLE_MS small), well before this stream's 8s EOF.
  update '{"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"working"}}'
  sleep 8
  exit 0
  ;;
