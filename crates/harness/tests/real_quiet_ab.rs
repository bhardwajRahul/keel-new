...
//!
//!   KEEL_AB=1 cargo test -p keel-harness --test real_quiet_ab -- --ignored --nocapture
//!
...
use keel_harness::{AcpHarness, CancellationToken, Harness, RunControls, SteerMessage};
use keel_proto::{AgentEvent, RunRequest, SandboxLevel, UserInputAnswer, UserInputQuestion};
...
fn init_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe { std::env::set_var("KEEL_ACP_QUIET_SETTLE_MS", QUIET_MS.to_string()) };
    });
}
