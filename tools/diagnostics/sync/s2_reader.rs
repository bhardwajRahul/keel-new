//! Fresh RUST reader for an s2 loro room — the exact client path every keel
...
//! Usage: cargo run -p keel-sync --example s2_reader -- <wsUrl> <roomId>
use loro::LoroDoc;
...
async fn main() {
    ...
    let doc = LoroDoc::new();
    let client = keel_sync::RoomClient::connect(url, room_id, doc.clone())
        .await
        .expect("connect");
    ...
    let session = keel_doc::SessionDoc::from_doc(doc.clone());
