//! `keel daemon …` — install/manage `keel headless` as a background service:
...
//! `KEEL_*` environment captured at install time, so
//! `KEEL_EDGE_URL=… keel daemon install` bakes that override in.
//!
//! Auth is decoupled: without a saved session the service remains up on the
//! local-only profile. `keel login` and a service restart opt into sync.
...
const LAUNCHD_LABEL: &str = "app.keel.daemon";
/// Same unit name the curl|sh installer (`edge/src/install.sh`) writes, so
/// `keel daemon …` manages that installation rather than a competing copy.
const SYSTEMD_UNIT: &str = "keel.service";
...
/// default PATH won't find); the `KEEL_*`/logging vars only when set.
const CAPTURED_ENV: &[&str] = &[
    ...
    "KEEL_EDGE_TOKEN",
    "KEEL_ORG_ID",
    "KEEL_WORKOS_CLIENT_ID",
    "KEEL_WORKOS_API_BASE",
    "KEEL_IPC_PORT",
    "KEEL_CALLBACK_PORT",
    "KEEL_HARNESS",
    "KEEL_DEVICE_NAME",
...
pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the keel executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("keel daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `keel daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `keel daemon start`)"
                } else {
                    " (not installed — `keel daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Keel headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
    ...
    unit.push_str(&format!(
        "ExecStart={} headless\nRestart=on-failure\nRestartSec=5\nEnvironmentFile=-%h/.keel/env\n\n[Install]\nWantedBy=default.target\n",
        systemd_exec_path(exe)
    ));
    unit
}

/// The ExecStart binary path. An exe under `~/.keel/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".keel/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.keel/app/current/keel".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            Path::new("/usr/local/bin/keel"),
            &[
                ("PATH".into(), "/usr/bin:/bin".into()),
                ("KEEL_EDGE_URL".into(), "https://edge.example".into()),
                ("RUST_LOG".into(), "info,keel=\"debug\"".into()),
            ],
        );
        assert!(unit.contains("ExecStart=/usr/local/bin/keel headless\n"));
        assert!(unit.contains("Environment=\"PATH=/usr/bin:/bin\"\n"));
        assert!(unit.contains("Environment=\"KEEL_EDGE_URL=https://edge.example\"\n"));
        // Inner quotes escaped so systemd re-parses the value verbatim.
        assert!(unit.contains("Environment=\"RUST_LOG=info,keel=\\\"debug\\\"\"\n"));
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.keel/env"));
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.keel/app/0.3.0/keel"),
                Some(Path::new("/home/u")),
            ),
            "%h/.keel/app/current/keel"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/keel"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/keel"
        );
    }
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            Path::new("/Users/x/keel & co/keel"),
            &[("KEEL_EDGE_URL".into(), "https://e?a=1&b=2".into())],
            Path::new("/Users/x/.keel/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>app.keel.daemon</string>"));
        // XML-escaped exe path and env value.
        assert!(plist.contains("<string>/Users/x/keel &amp; co/keel</string>"));
        ...
        assert!(
            plist.contains("<key>StandardOutPath</key><string>/Users/x/.keel/daemon.log</string>")
        );
    }
}
