//! `keel login` / `keel logout` / `keel status` — the standalone auth surface.
//!
//! Sign-in used to live only inside `keel headless`, coupling authentication to
//! the long-running daemon. These commands work on the persisted session
//! (`{data_dir}/session.json`) and exit, so a service-managed `keel headless`
...
use keel_engine::{AuthState, Engine, EngineConfig, InstanceLock, WorkspaceScope};
    ...
    match scope {
        ...
        WorkspaceScope::Synced => match auth {
            ...
            AuthState::NeedsOrganization { user } => AccountStatus {
                mode: "synced",
                auth: format!(
                    "signed in as {} but no workspace selected — run `keel login`",
                    user.email
                ),
                healthy: false,
            },
            AuthState::SignedOut => AccountStatus {
                mode: "synced",
                auth: "saved session is no longer valid — run `keel login`".into(),
                healthy: false,
            },
        },
    }
...
/// `keel login`: authenticate via the paste-code flow (and workspace
...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if let AuthState::SignedIn { user, org_id } = auth.state() {
        ...
        println!("Run `keel logout` first to switch accounts.");
        println!("The next engine start will use the synced workspace.");
        return Ok(());
    }
    if !std::io::stdin().is_terminal() {
        anyhow::bail!("keel login needs an interactive terminal");
    }
    keel_engine::terminal_sign_in(&auth).await?;
    match auth.state() {
        AuthState::SignedIn { user, org_id } => {
            ...
            println!(
                "Sync is ready. Start or restart Keel to open the synced workspace; existing local sessions will stay local."
            );
        }
...
/// `keel logout`: remove the persisted session.
...
/// `keel status`: report the fixed scope a new engine would select, optional
/// auth, and engine liveness. Local-only is a healthy signed-out state.
pub async fn status(config: EngineConfig) -> anyhow::Result<()> {
    let auth = Engine::build_auth(&config).await;
    let next_scope = Engine::initial_workspace_scope_for_config(&auth, &config);
    let scope = live_engine_scope(config.ipc_port)
        .await
        .unwrap_or(next_scope);
    let account = if scope == WorkspaceScope::Local
        && config.workos_client_id.is_none()
        && config
            .edge_token
            .as_deref()
            .is_none_or(|token| token.trim().is_empty())
    {
        AccountStatus {
            mode: "local only",
            auth: "no cloud account configured".into(),
            healthy: true,
        }
    } else {
    ...
    println!(
        "Edge:     {}",
        if config.workos_client_id.is_none() && config.edge_token.is_none() {
            "disabled (local-only)"
        } else {
            &config.edge_url
        }
...
async fn live_engine_scope(ipc_port: u16) -> Option<WorkspaceScope> {
    let client = keel_rpc::connect_ws(&format!("ws://127.0.0.1:{ipc_port}"))
        .await
        .ok()?;
    let value = client
        .call(keel_rpc::methods::ENGINE_INFO, serde_json::json!({}))
        .await
        .ok()?;
    serde_json::from_value::<keel_engine::EngineInfo>(value)
        .ok()
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`keel daemon stop`, or quit the Keel app), or use the running UI instead."
        )
    })
}
...
mod tests {
    use super::*;
    use keel_engine::{AuthUser, HarnessId};
